...
export async function queryOneInch(
  ...
  const { linear, chainId } = { linear: true, chainId: ChainId.MAINNET, ...opts }
  ...
  query += `&protocols=${protocols.join(',')}`
  query += `&slippage=10`
  query += `&disableEstimate=true`
  query += linear ? `&mainRouteParts=1` : ''
  query += linear ? `&parts=1` : ''

  const response = await axios.get<IOneInchResponse>(query).catch(() => null)

  if (response?.status != 200) {
    throw new Error(`Query ${query} failed with error: ${JSON.stringify(response)}`)
  }
...
export async function findBestUniswapV3Route(
  ...
  const oneInchResponse = await queryOneInch(tokenIn.address, tokenOut.address, amountIn, PROTOCOL.UNISWAP_V3)

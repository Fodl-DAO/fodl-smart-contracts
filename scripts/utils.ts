...
export async function sendToken(tokenDataOrContract: TokenData | IERC20, recipient: string, amount?: BigNumberish) {
  ...
  switch (token.address.toLowerCase()) {
    ...
    case DOLA.address.toLowerCase():
      ...
      break
    case Tokens.SUSHI.address.toLowerCase():
      const sushiOwner = await Ownable__factory.connect(tokenData.address, ethers.provider)
        .owner()
        .then(impersonateAndFundWithETH)

      if (!amount) amount = float2SolidityTokenAmount(tokenData, defaultVolatileCoinAmount)

      await IWBTC__factory.connect(tokenData.address, sushiOwner).mint(recipient, amount)

      break
    case Tokens.CRV.address.toLowerCase():
      const crvMinter = await ICRV__factory.connect(tokenData.address, ethers.provider)
        .minter()
        .then(impersonateAndFundWithETH)

      if (!amount) amount = float2SolidityTokenAmount(tokenData, defaultVolatileCoinAmount)

      await ICRV__factory.connect(tokenData.address, crvMinter).mint(recipient, amount)
      break
    default:
      await impersonateAndFundWithETH(providerAddress)
      ...
      if (!amount) amount = balance
        ...
        const amountBN =
          typeof amount === 'number' ? float2SolidityTokenAmount(tokenData, amount) : BigNumber.from(amount)

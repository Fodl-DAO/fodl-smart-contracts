...
const MATIC_PRINCIPAL_AMOUNT = 100_000
const USD_PRINCIPAL_AMOUNT = 100_000

const MAX_DEVIATION = 0.4

describe('SimplePositionPolygonFoldingConnector', () => {
  const inAave = {
  ...
  const fixture = deployments.createFixture(async (hre: HardhatRuntimeEnvironment) => {
    ...
    await sendToken(POLY.WMATIC(), alice.address, MATIC_PRINCIPAL_AMOUNT)
    // await sendToken(POLY.USDT(), alice.address, USD_PRINCIPAL_AMOUNT)
    // await sendToken(POLY.DAI(), alice.address, USD_PRINCIPAL_AMOUNT)
    await sendToken(POLY.USDC(), alice.address, USD_PRINCIPAL_AMOUNT)

...
import { formatEther, formatUnits, hexZeroPad, parseUnits } from 'ethers/lib/utils'
...
export function solidityTokenAmount2Float(tokenOrDecimals: TokenData | number, value: BigNumberish) {
  if (typeof value === 'number') return value
  const decimals = typeof tokenOrDecimals === 'number' ? tokenOrDecimals : tokenOrDecimals.decimals
  return parseFloat(formatUnits(value, decimals))
}

export function float2SolidityTokenAmount(tokenOrDecimals: TokenData | number, float: number) {
  const decimals = typeof tokenOrDecimals === 'number' ? tokenOrDecimals : tokenOrDecimals.decimals
  return parseUnits(`${float.toFixed(decimals)}`, decimals)
}
...
export function scaleByMantissa(value: BigNumber) {
  return value.mul(MANTISSA)
}

export function convertPrice(
  price: BigNumber | number,
  decimalsOfDenominatorToken: number,
  decimalsOfNumeratorToken: number
): number | BigNumber {
  if (typeof price === 'number') {
    return parseUnits(`${price.toFixed(decimalsOfNumeratorToken)}`, decimalsOfNumeratorToken)
      .mul(MANTISSA)
      .div(parseUnits('1', decimalsOfDenominatorToken))
  }

  return parseFloat(
    formatEther(
      price
        .mul(BigNumber.from(10).pow(decimalsOfDenominatorToken))
        .div(BigNumber.from(10).pow(decimalsOfNumeratorToken))
    )
  )
}

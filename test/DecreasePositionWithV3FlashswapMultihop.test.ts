...
import {
  ...
  SUBSIDY_HOLDER_ADDRESS_MAINNET,
  SUBSIDY_PRINCIPAL,
...
describe('DecreasePositionWithV3FlashswapMultihopConnector', () => {
  ...
  const fixture = deployments.createFixture(async (hre: HardhatRuntimeEnvironment) => {
    const { amountOut_float: price } = await getCompoundQuote(platform, principalToken, borrowToken, 1)
  ...
  beforeEach('load fixture', async () => {
    ;({ account, sameTokenPositionAccount, compoundPriceOracleMock, alice, mallory, foldingRegistry, price } =
      await fixture())
  })
  ...
  describe('decreasePositionWithV3FlashswapMultihop', () => {
    describe('different token position', () => {
      ...
      it('allows to close position with bad debt', async () => {
        const priceChangeFactor = MANTISSA.div(1000)
        await compoundPriceOracleMock.setPriceUpdate(
          COMPOUND_TOKENS_TO_CTOKENS[principalToken.address],
          priceChangeFactor
        )
        ...
        expect(aliceBalanceDelta).to.be.gte(0)
        expect(subsidyBalanceDelta).to.be.gte(0).lt(aliceBalanceDelta)
